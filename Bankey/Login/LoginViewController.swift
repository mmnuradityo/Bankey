...
extension LoginViewController {
    ...
    private func animate() {
        ...
        let animatorTitleAnchor = UIViewPropertyAnimator(duration: duration, curve: .easeInOut) {
            self.titleLeadingAnchor?.constant = self.leadingEdgeOnScreen
            self.view.layoutIfNeeded()
        }
        animatorTitleAnchor.startAnimation()
        
        let animatorSubtitleAnchor = UIViewPropertyAnimator(duration: duration, curve: .easeInOut) {
            self.subTitleLeadingAnchor?.constant = self.leadingEdgeOnScreen
            self.view.layoutIfNeeded()
        }
        animatorSubtitleAnchor.startAnimation(afterDelay: 0.2)
        
        let animatorAlpha = UIViewPropertyAnimator(duration: duration * 2.0, curve: .easeInOut) {
            self.titleLabel.alpha = 1
            self.subtitleLabel.alpha = 1
            self.view.layoutIfNeeded()
        }
        animatorAlpha.startAnimation(afterDelay: 0.2)
    }
